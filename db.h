...
//contains column info. 
//rows are arrays of cd_entries
//cd_entry contains meta information of the column data. type (string, int) length in bytes (int will always be length 4), etc
...
typedef struct tpd_list_def //list of tpd_entries
{
	int				list_size;
	int				num_tables;
	int				db_flags;
	tpd_entry	tpd_start; //gets overlapped by first valid tpd_entry
}tpd_list;
...
{
	int file_size; //total row size * 100
	int record_size; //size of 1 row?
	int num_records; //number of rows
	int record_offset; //offset for last row
	int file_header_flag; //dunno
	tpd_entry *tpd_ptr; 
} table_file_header;
...
typedef enum error_return_codes
{
  ...
  /* Must add all the possible errors from I/U/D + SELECT here */
    MISSING_VALUES_KEYWORD, //-387
	MISSING_OPEN_PAREN, //-386
	MISSING_CLOSING_PAREN, //-385
	EMPTY_VALUES, //-384
	INVALID_SYNTAX, //-383
	END_OF_INPUT, //-382
	CANNOT_BE_NULL, //-381
	INVALID_DATA_TYPE, //-380
	STRING_OVERFLOW, //-379
	INTEGER_OVERFLOW, //-378
...
//added
int sem_insert(token_list *tok_list);
int sem_select(token_list *tok_list);
int sem_delete(token_list *tok_list);
int sem_update(token_list *tok_list);
/*
